(function (root, factory) {
  ...
  function classSkillSpecFromData(raw, positions, classId) {
    ...
    return {
      ...
      blocksByTerrain: ["linear", "arc"].includes(everrealm.delivery_mode),
      blocksByUnits: everrealm.delivery_mode === "linear",
      stopOnFirstUnit: everrealm.delivery_mode === "linear",
      piercing: everrealm.piercing === true || original.piercing === true,
      maxPierce: everrealm.max_pierce == null ? null : Math.max(1, Math.trunc(Number(everrealm.max_pierce) || 1)),
      friendlyFire: everrealm.friendly_fire === true,
  ...
  function cloneSkill(source) {
    ...
    return {
      ...
      piercing: source.piercing === true,
      maxPierce: source.maxPierce == null ? null : Math.max(1, Math.trunc(Number(source.maxPierce) || 1)),
