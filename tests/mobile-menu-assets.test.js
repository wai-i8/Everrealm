...
test("icon rail resets the legacy text-menu grid before sizing the artwork", () => {
  const finalRailRuleStart = responsiveCss.lastIndexOf("#exploreSidebar .sidebar-primary .explore-menu-button,");
  const finalRailRule = responsiveCss.slice(finalRailRuleStart, responsiveCss.indexOf("\n}", finalRailRuleStart) + 2);
  ...
  assert.match(responsiveCss, /transform:\s*scale\(1\.06\) !important/);
});
