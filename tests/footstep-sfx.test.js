...
test("stone maps use four footstep variants while field maps stay silent", () => {
  assert.equal(Footsteps.SURFACE_BY_MAP.world, "stone");
  assert.equal(Footsteps.SURFACE_BY_MAP.inn, "stone");
  assert.equal(Footsteps.SURFACE_BY_MAP.field, undefined);
...
test("mute, zero volume and unsupported surfaces suppress footsteps", () => {
  let plays = 0;
  ...
  const controller = Footsteps.createFootstepController({
    ...
    createAudio: () => ({ load() {}, cloneNode: () => ({ play() { plays++; return Promise.resolve(); } }) }),
  });
  controller.setMap("world");
    ...
    controller: {
      ...
      createAudio(src) {
        return {
          ...
          load() {},
          cloneNode() {
            return { volume: 0, playbackRate: 1, currentTime: 0, play() { plays++; return Promise.resolve(); } };
          },
        };
      },
    },
