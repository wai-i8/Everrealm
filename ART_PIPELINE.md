...
Main Town runtime 直接以呢個 `7680 × 4320` native world coordinate space 繪製 master art；地圖尺寸係 1:1 world pixels，標準 locomotion atlas 喺呢張 native scene 使用 `unitScale: 1`，即來源 frame 嘅 authored pixel 直接對應 Main Town world pixel。唔可以再加一層 legacy compact-map scale、CSS 放大或 per-map image transform。相機係 viewport window：每幀以 camera center 同 viewport world size 從 native image 取 source crop，直接 draw 到 final Canvas backing output，唔先將全圖 downsample 到中間 gameplay canvas。相機 zoom 只負責玩家視角（far／mid／near）與 viewport responsive framing，必須同 navigation、feet pivot、click-to-world inverse 使用同一個 camera transform；DPR 只按 CSS viewport 放大 backing store／輸出採樣密度，唔改變 world size 或 crop。其他 legacy procedural／interior scene 如果未提供 native `unitScale`，保留 locomotion contract 嘅既有 default，避免將 Main Town source scale 假設套去舊圖。
