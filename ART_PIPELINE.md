...
Main Town runtime 直接以呢個 `7680 × 4320` native world coordinate space 繪製 master art；地圖尺寸係 1:1 world pixels。所有場景共用同一個 Standard Mobile Unit locomotion projection，atlas cell 像素直接對應 authored world pixels，唔存在 `worldScale`、`entityScale`、`unitScale` 或其他 migration render factor。玩家大小只由標準 atlas cell／anchor 同 global camera zoom 決定，唔由 Main Town、室內圖尺寸或圖片解析度決定。唔可以再加一層 legacy compact-map scale、CSS 放大或 per-map image transform。相機係 viewport window：每幀以 camera center 同 viewport world size 從 native image 取 source crop，直接 draw 到 final Canvas backing output，唔先將全圖 downsample 到中間 gameplay canvas。相機 zoom 只負責玩家視角（far／mid／near）與 viewport responsive framing，必須同 navigation、feet pivot、click-to-world inverse 使用同一個 camera transform；DPR 只按 CSS viewport 放大 backing store／輸出採樣密度，唔改變 world size 或 crop。細地圖若小過 camera viewport，viewport 外只填黑色，唔拉伸 scene。
