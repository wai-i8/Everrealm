(function (root, factory) {
  ...
  function facilityCopy(tab, context) {
    return {
      ...
      missions: ["", "任務", ""],
      bag: ["", "物品欄", ""],
      equipment: ["", "角色裝備欄", "查看身上裝備與已擁有收藏，隨時切換出戰配置。"],
      deck: ["", context === "deck" ? "面板配置" : "面板", ""],
      guild: ["", "公會委託", "同一時間只能進行一份委託；完成目標後返回公會回報。五份固定委託皆可重複接受，開啟信封後會獲得對應星級的技能書。"],
      shop: ["", context === "general-store" ? "道具店" : "裝備店", context === "general-store" ? "" : "同一間店可以購買格鬥士武器與防具；用分類切換武器、頭部、上身、下身及武道服。"],
      skills: ["", "技能樹", ""],
      codex: ["", "怪物圖鑑", "記錄你曾遇見及擊敗的每一種怪物。"],
    }[tab];
  }
  ...
  function wireWindow(state, handlers) {
    ...
    state.panel.addEventListener("click", (event) => {
      ...
      if (event.target.closest(".inventory-detail-layer")) {
        handlers.contentClick(event, state);
        return;
      }
