(function (root, factory) {
  ...
  if (!classData?.LEVEL_CAP || !classData?.LEVEL_EXP_REQUIREMENTS) {
    throw new Error("EverrealmClassData must be loaded before rpg-core.js");
  }

  const TAU = Math.PI * 2;
  const LEVEL_CAP = classData.LEVEL_CAP;
  const LEVEL_EXP_REQUIREMENTS = classData.LEVEL_EXP_REQUIREMENTS;
