...
- `C:\Users\lauka\Projects\Everrealm` 的 `main` 是穩定整合工作樹；正常單線 task 預設直接在乾淨 `main` 修改、驗證並 commit，完成後不可留下 dirty `main`。
- 同一 thread／queue 的多個 task 是順序執行：後一個 task 必須從前一個 task 已提交後的最新 `main` 開始。開始任何修改前仍必須完成 Git preflight；如果 `main` dirty，立即停止並回報，不可 stash、reset、clean、覆蓋或替未知修改 commit。
- Worktree／feature branch 只在用戶明確要求、明確要求真正並行隔離開發，或 task 已明確獲准需要 branch isolation 時使用；不可因為 queue、backlog 或存在其他 task 就自行建立。詳細流程見 `docs/DEVELOPMENT_WORKFLOW.md`。
...
- Equipment schema, canonical slots and equipment-side combat modifiers:
...
- Direct-main task must complete its relevant automated tests, feature-specific runtime smoke and visual verification before commit; after commit verify `main` is clean.
- For explicitly isolated feature branches, complete the same feature verification before integration. Merge completed branches one at a time, run `git diff --check` and targeted smoke after each merge, then run `npm test` once after the batch and confirm `runtimeErrors: 0` and clean `main`.
