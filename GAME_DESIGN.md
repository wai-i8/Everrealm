...
- 非戰鬥探索分為兩種 presentation contract。一般 native scene 仍可使用以 camera crop 為核心嘅探索視角；但凡係由完整 master artwork 加 matching walkable authoring 圖定義、並採用 compact full-map exploration 嘅地圖，主畫面必須將完整場景等比例縮入細型 gameplay world，玩家可以喺整張圖上導航，唔用 player-centred native crop。呢個 compact transform 必須同時作用於背景、玩家、NPC、怪物、transition、interaction、碰撞、pathfinding 同 click-to-world conversion，唔可以只縮背景。Desktop 探索用滑鼠滾輪連續調整視角，手機用雙指 pinch 連續調整；全局 camera zoom 範圍係 `0.35` 至 `1.0`，唔設固定中間 preset。右上圓形小地圖採玩家置中嘅 local view；左側主功能列保留狀態、物品、面板、任務、系統，好友／隊伍等社交入口放右下角並與左側 launcher、手機收合 tab 共用同一尺寸 token；手機自動收合 launcher 時社交入口亦一併收合。技能樹由「面板」視窗內進入，唔再佔用獨立探索 icon。
...
- 左側功能列保持原作式窄身、單欄及極簡；每個彈出頁只處理當前主題，不再重複放公會摘要或跨頁分頁列。開啟狀態、物品、面板、任務或系統時左側功能列繼續顯示，玩家可以同時開多個不同功能 window；同一功能只維持一個 instance。技能樹由面板視窗內開啟。新開／重新點擊嘅 window 置頂，點擊其他已開 window 會將其帶回前景。系統設定同其他主功能使用同一套可拖動、可疊放 window 行為，唔再固定黐住 system icon。
...
- 學習成功才消耗技能書；前置不足、職業不符或條件未滿時保留物品。已學會技能的重複技能書亦會保留在物品欄，不會自動轉換成其他資源。
...
- Authoritative map transitions begin an immediate full-stage fade/cover while server validation runs in parallel. The destination map is only committed after validation succeeds; the cover holds through one rendered destination frame, then fades away. In a party transition, each client may resume local exploration as soon as its own destination load is complete and its transition-ready acknowledgement succeeds; encounter/battle start remains blocked until the shared party transition itself is fully ready. If validation fails, the original scene is revealed again and the normal system error path remains authoritative.
...
- Online exploration uses the same left-click / tap gesture on desktop and phone to select another realtime player. Only the remote player's visible name plus a small head/upper-body screen-space hit region is interactive; the lower body/feet remain ground-click territory so crowds cannot block movement. Selection opens a compact social popup for profile, friend, unrestricted one-to-one `密語`, trade and party actions. Friend/trade/party invitations share one outgoing waiting lock and visible cancel card. Party gameplay still uses authoritative delayed network coordinates, while the client may render party members in a presentation-only formation around the local player so the normal ~0.5 s sync buffer does not look like followers are constantly lagging behind.
