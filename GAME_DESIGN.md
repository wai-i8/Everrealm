...
- Online exploration uses the same left-click / tap gesture on desktop and phone to select another realtime player. Only the remote player's visible name plus a small head/upper-body screen-space hit region is interactive; the lower body/feet remain ground-click territory so crowds cannot block movement. Selection opens a compact social popup. V1 social supports profile preview, confirmed friend requests and friend-only one-to-one `密語`; trading and party/shared-combat actions are deferred. Detailed storage and security rules live in `docs/SOCIAL_SYSTEM.md`.
