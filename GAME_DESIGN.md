...
- 玩家長時間無操作不會再開啟阻塞式「停一停／Night Watch Paused」視窗；持久化改用無干擾的 dirty-state autosave checkpoint。物品欄採用「選取物品 → 查看詳情 → 執行可用動作」流程，保留原有使用、換裝及技能書行為。
...
- 公會委託板的固定 V1 目錄、星級信封及討伐／送信流程見 `docs/GUILD_COMMISSION_SYSTEM.md`；信封開出具名技能書，但不會直接學會，亦不繞過 Fighter 前置。
