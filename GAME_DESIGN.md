...
- 非戰鬥探索分為兩種 presentation contract。一般 native scene 仍可使用以 camera crop 為核心嘅探索視角；但凡係由完整 master artwork 加 matching walkable authoring 圖定義、並採用 compact full-map exploration 嘅地圖，主畫面必須將完整場景等比例縮入細型 gameplay world，玩家可以喺整張圖上導航，唔用 player-centred native crop。呢個 compact transform 必須同時作用於背景、玩家、NPC、怪物、transition、interaction、碰撞、pathfinding 同 click-to-world conversion，唔可以只縮背景。Desktop 探索用滑鼠滾輪連續調整視角，手機用雙指 pinch 連續調整；全局 camera zoom 範圍係 `0.35` 至 `1.0`，唔設固定中間 preset。右上圓形小地圖採玩家置中嘅 local view；狀態、物品、裝備、DECK、技能樹等入口全部放在左側。
