...
玩家可決策嘅移動規劃及行動規劃階段各有 **30 秒**。倒數完仍未提交就視為「待機」：移動階段不移動，行動階段不出招。移動規劃 + 行動規劃完成後先計作一個完整 Round，HUD 倒數以 `Round N　30s` 格式顯示。

單人 PVE 若玩家離線，server 唔會長開背景 timer；重新連線時按 authoritative deadline 一次過 fast-forward 已過期嘅 30 秒 phase。玩家每個過期 phase 都自動待機，怪物繼續用正常 AI／resolver 行動；一旦玩家已倒下就停止補算並進入既有戰敗／復活流程，否則以補算後嘅 HP、位置及 Round 繼續原戰鬥。
