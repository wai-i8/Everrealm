...
Player and ordinary-unit render size, exploration movement speed, and the Near/Mid/Far camera presets (`0.78` / `1.0` / `1.22`) are global contracts. Native exploration atlas pixels are authored world pixels; no global `worldScale`, `entityScale`, `unitScale` or equivalent migration factor may shrink them. These contracts do not read map dimensions, scene identity, or source resolution. Map dimensions only constrain world bounds and camera cropping. When a camera viewport is larger than a small map, the renderer leaves the area outside the native map black; it never stretches the map or enlarges its units to fill the screen.
