...
接待員服務使用共用 authored-region interaction：`3344 × 1882` scene 係基準 interior resolution 嘅 `2×`，因此由 NPC magenta region 最近點計算 `320 px` reach，點擊命中使用 `36 px` padding；名稱亦以 magenta region 上緣定位，而唔再用 semantic feet point 壓住角色面部。
