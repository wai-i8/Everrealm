...
Main Town uses the same global player render scale, exploration movement speed, and Near/Mid/Far camera presets (`0.78` / `1.0` / `1.22`) as every other map. Native exploration atlas pixels are authored world pixels with no `worldScale`, `entityScale`, `unitScale` or equivalent migration factor. Its large native dimensions affect only world bounds and how far the camera can travel; there is no Main-Town-specific entity scale, player override, speed multiplier, zoom compensation, or fit-to-map conversion.
