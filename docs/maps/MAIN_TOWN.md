...
Main Town uses the same canonical player and monster entity dimensions, `330` world-units/sec base exploration speed, and global Far/Mid/Near native-world camera presets (`0.46176` / `0.592` / `0.72224`) as every other map. Its large native dimensions affect only world bounds and how far the camera can travel; there is no Main-Town-specific entity scale, player override, speed multiplier, zoom compensation, fit-to-map conversion, or legacy logical-world projection. Changing the Main Town source resolution must have no effect on player, NPC, outdoor monster, battle actor or skill-animation size.
