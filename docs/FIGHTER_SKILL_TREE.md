...
Each executed strike is separated by the shared battle strike interval (currently about `0.5s` in normal motion), so the action duration expands with the number of executed strikes instead of compressing every hit into the old fixed action window.
