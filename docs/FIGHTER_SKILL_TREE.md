...
完整 machine-readable data 由 `data/skills/fighter.js` 載入 `EverrealmFighterSkillData`，再由 `skill-core.js` 建立 immutable `SKILL_CATALOG`。`skills[]` 每一招均保存：
...
`kentotsu` is `3 AP = 1.0×`. Only an authored non-damage control/status utility applies `0.8`; multi-hit count, range, speed, delivery and height do not. Fixed HP skills use their explicit `source_defined_fixed_damage` model and never also apply the square-root formula. Multi-hit totals are calculated once, then split with remainder assigned to later hits (`splitDamageLaterHits`). Every hit then rolls accuracy independently and the action animation repeats once per executed `hit_count`; its sound, floating result and Battle Log entry are emitted in the same per-hit order. If the route has no candidate after an earlier kill, the remaining empty Hits produce no animation, sound, floating result or log entry. This is one shared resolver for all six Fighter multi-hit skills: `rendan` (2), `korendan` (3), `byakkorendan` (5), `lusedes_da` (3), `lusedes_koku` (8), and `lusedes_tan` (6).
Each executed strike is separated by the shared battle strike interval (currently about `0.5s` in normal motion), so the action duration expands with the number of executed strikes instead of compressing every hit into the old fixed action window. Damage is applied when each strike is presented, keeping HP bars, hit reactions, floating numbers, Battle Log entries and sound in the same per-strike timing.
...
`range_cells_relative` selects the intended target; it never substitutes for attack path. For every facing, runtime rotates `[lateral, depth]` deterministically. All attack skills, including close-range bite／strike／slam／slash skills, use the shared `facingOrthogonalPriority` route unless they are explicitly pathless. Linear skills use forward-first routing in the forward half, lateral-first in the rear half, and lateral-only on the same row. `traceAttackPath` returns ordered living route candidates after terrain／arc checks. A successful friendly candidate is the actual impact and takes damage under Everrealm's fixed `friendlyFire=true` rule; non-piercing attacks stop there. Pathless skills retain their authored effect area and have no intermediate interception.
